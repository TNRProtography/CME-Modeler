    ...
    <div className="col-span-12 card bg-neutral-950/80 p-5 border border-cyan-800/40">
      <div className="flex flex-wrap justify-between gap-3 items-center mb-3">
        ...
        <span className="text-xs px-2 py-1 rounded bg-cyan-900/40 text-cyan-200 border border-cyan-700/50">
          Confidence {(analysis.confidenceR2 * 100).toFixed(0)}%
        </span>
      </div>

      <p className="text-sm text-neutral-300 mb-3">{analysis.explanation}</p>
      <FluxRopeSlinky analysis={analysis} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mt-4">
        <div className="rounded-lg border border-neutral-700/60 bg-neutral-900/60 p-3">
          ...
          <div className="text-sm text-neutral-100 font-semibold mt-1">{ORIENTATION_LABELS[analysis.orientation]}</div>
          <div className="text-xs text-neutral-400 mt-2">Estimated progress: {analysis.progressPct.toFixed(0)}%</div>
        </div>
        <div className="rounded-lg border border-neutral-700/60 bg-neutral-900/60 p-3">
          <div className="text-xs text-neutral-400">Current IMF</div>
          <div className={`text-sm font-semibold mt-1 ${bzColor(analysis.nowBz)}`}>Bz {analysis.nowBz.toFixed(1)} nT</div>
          <div className="text-xs text-neutral-300 mt-1">Bt {analysis.nowBt.toFixed(1)} nT · Rotation {analysis.rotationDegPerHour.toFixed(1)}°/hr</div>
        </div>
        <div className="rounded-lg border border-neutral-700/60 bg-neutral-900/60 p-3">
          <div className="text-xs text-neutral-400">What to expect</div>
          <div className="text-sm text-neutral-100 mt-1">
            {analysis.predictedTurnNorthAt
              ? `Likely Bz polarity turn around ${new Date(analysis.predictedTurnNorthAt).toLocaleTimeString('en-NZ', { hour: '2-digit', minute: '2-digit', hour12: false })}.`
