...
const ForecastModelsModal: React.FC<ForecastModelsModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const fetchEnlilImages = async () => {
      ...
      try {
        // Fetch a list of filenames from the NOAA text file directory listing
        ...
        setEnlilImageUrls([]); // Clear on error
      } finally {
    ...
    fetchEnlilImages();
    // Cleanup function for when the modal is closed before fetch completes
    return () => {
        if (intervalRef.current) clearInterval(intervalRef.current);
    }
  }, [isOpen]);
  ...
  useEffect(() => {
    // This effect runs when the image URLs are successfully fetched
    if (enlilImageUrls.length > 0) {
      intervalRef.current = window.setInterval(() => {
        setCurrentEnlilImageIndex(prevIndex => (prevIndex + 1) % enlilImageUrls.length);
      }, 100); // Change image every 100ms for a ~10fps animation
    }
        ...
        <div className="overflow-y-auto p-5 styled-scrollbar pr-4 grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <section className="space-y-4">
            ...
            <div className="space-y-4">
                <a href="https://huxt-bucket.s3.eu-west-2.amazonaws.com/wsa_huxt_forecast_latest.png" target="_blank" rel="noopener noreferrer" className="block bg-neutral-900 p-2 rounded-lg hover:ring-2 ring-sky-400 transition-shadow">
                ...
                <a href="https://huxt-bucket.s3.eu-west-2.amazonaws.com/wsa_huxt_animation_latest.mp4" target="_blank" rel="noopener noreferrer" className="block bg-neutral-900 p-2 rounded-lg hover:ring-2 ring-sky-400 transition-shadow">
          ...
          <section className="space-y-4">
            ...
            <div className="bg-neutral-900 p-2 rounded-lg relative min-h-[300px] flex items-center justify-center">
