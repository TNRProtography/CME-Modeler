 ...
 * Consolidates the main "Forecast" score and the "Substorm Forecast" into a single card.
 *
 * NOTE: This file intentionally provides BOTH:
 *   export default CombinedForecastPanel;
 *   export { CombinedForecastPanel };
 * so it works with either default or named imports.
 */
...
const visibilityMeaning = (score: number | null | undefined) => {
  ...
  if (s < 10)
    return { emoji: "😞", title: "Little to no auroral activity", advice: "Low chance right now. Monitor updates." };
  if (s < 25)
    return { emoji: "😐", title: "Minimal activity likely", advice: "Maybe a very faint glow. Dark skies help." };
  if (s < 40)
    return { emoji: "😊", title: "Camera-clear; sometimes naked-eye", advice: "Check a dark southern horizon; look for subtle motion." };
  if (s < 50)
    return { emoji: "🙂", title: "Faint naked-eye glow possible", advice: "Be patient; let eyes adapt 5–10 minutes." };
  if (s < 80)
    return { emoji: "😀", title: "Good chance of visible color", advice: "Head darker; waves/brightenings likely." };
  return { emoji: "🤩", title: "High probability of significant substorms", advice: "Watch mid-sky to high south; dynamic activity likely." };
};
...
function CombinedForecastPanel({
  ...
  const statusLabel = typeof status === "string" ? status.replace(/_/g, " ") : "—";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-black/30 border border-neutral-700/30 rounded-xl p-5">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="text-sm text-neutral-400">{lastUpdated || "Updating…"}</div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl bg-black/30 border border-neutral-700/30 p-4">
            ...
            <div className="text-xs text-neutral-500 mt-1">Status: {statusLabel}</div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="mt-2 h-2.5 w-full rounded-full bg-neutral-800 overflow-hidden">
                <div
                  className={`h-full bg-gradient-to-r ${likelihoodGradient(likePct)}`}
                  style={{ width: `${likePct}%` }}
                />
              </div>
            </div>
