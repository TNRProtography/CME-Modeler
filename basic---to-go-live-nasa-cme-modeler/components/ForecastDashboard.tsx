...
const getSuggestedCameraSettings = (score: number | null, isDaylight: boolean) => {
  ...
  return {
    ...
    phone: {
      android: { iso: "3200-6400 (Max)", shutter: "15-30s", aperture: "Lowest f-number", focus: "Infinity", wb: "Auto or 3500K-4000K" },
      apple:   { iso: "Auto (max Night Mode)", shutter: "Longest Night Mode (10-30s)", aperture: "N/A (fixed)", focus: "Infinity", wb: "Auto or 3500K-4000K" }
    },
    dslr: { iso: "3200-6400", shutter: "15-25s", aperture: "f/2.8-f/4 (widest)", focus: "Manual to Infinity", wb: "3500K-4500K" }
  };
};

// Substorm panel (unchanged from your version)
...
const ForecastDashboard: React.FC<ForecastDashboardProps> = ({ setViewerMedia, setCurrentAuroraScore, setSubstormActivityStatus, navigationTarget }) => {
  const {
    ...
    goes18Data, goes19Data, loadingMagnetometer, substormForecast, nzMagnometer // NEW
  ...
  const tooltipContent = useMemo(() => ({
    ...
    'bz': `<strong>What it is:</strong> This is the most important ingredient for an aurora. Earth is protected by a magnetic shield. The 'Bz' value tells us the North-South direction of the Sun's magnetic field.<br><br><strong>Effect on Aurora:</strong> Think of Bz as the 'master switch'. When Bz points <strong>South (a negative number)</strong>, it's like a key turning in a lock. It opens a door in Earth's shield, allowing energy and particles to pour in. When Bz is North (positive), the door is closed. <strong>The more negative the Bz, the better the aurora!</strong>`,
  ...
  return (
    <div className="w-full h-full bg-neutral-900 text-neutral-300 relative" style={{ backgroundImage: `url('/background-aurora.jpg')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
      ...
      <div className="w-full h-full overflow-y-auto p-5 relative z-10 styled-scrollbar">
        <div className="container mx-auto">
          ...
          <main className="grid grid-cols-12 gap-6">
            ...
            {/* NEW: Collapsible NZ magnetometers box (hidden by default via chart’s “collapsed” UI) */}
            <div className="col-span-12">
              <div className="card bg-neutral-950/80 p-4">
                <NZMagnetometersChart
                  nzMagnometer={nzMagnometer}
                  timeRangeMs={magnetometerTimeRange}
                />
              </div>
            </div>
            ...
            <div className="col-span-12 grid grid-cols-1 lg:grid-cols-2 gap-6">
              <TipsSection />
              <CameraSettingsSection settings={useMemo(() => getSuggestedCameraSettings(auroraScore, isDaylight), [auroraScore, isDaylight])} />
            </div>
            ...
            <div className="col-span-12 card bg-neutral-950/80 p-4 flex flex-col">
              ...
              <div className="relative w-full" style={{paddingBottom: "56.25%"}}>
                <iframe title="Windy.com Cloud Map" className="absolute top-0 left-0 w-full h-full rounded-lg" src="https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=mm&metricTemp=°C&zoom=5&overlay=clouds&product=ecmwf&level=surface&lat=-44.757&lon=169.054" frameBorder="0"></iframe>
              </div>
            </div>
      ...
      <GraphModal
        ...
        nzMagnometer={nzMagnometer}         // NEW
        loadingMagnetometer={loadingMagnetometer}
        substormBlurb={{text: substormForecast.action, color: ''}}
