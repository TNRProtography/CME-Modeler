...
// This is the full forecast page, now as a React component
const ForecastDashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const GAUGE_API_ENDPOINTS = { power: 'https://hemispheric-power.thenamesrock.workers.dev/', speed: NOAA_PLASMA_ENDPOINT, density: NOAA_PLASMA_ENDPOINT, bt: NOAA_MAG_ENDPOINT, bz: NOAA_MAG_ENDPOINT };
        ...
        async function updateGauge(type: string) {
            ...
            const elements = { value: document.getElementById(`${type}-value`), emoji: document.getElementById(`${type}-emoji`), lastUpdated: document.getElementById(`${type}-last-updated`), bar: document.getElementById(`${type}-gauge-bar`) };
            if (!elements.value || !elements.emoji || !elements.lastUpdated || !elements.bar) return;
            try {
                if (type === 'power') { const response = await fetch(endpoint); if (!response.ok) throw new Error(`Fetch failed`); const data = await response.json(); const latestData = data.values[data.values.length - 1]; const value = parseFloat(latestData.value); const style = getGaugeStyle(value, type); elements.value.innerText = `${value.toFixed(1)} GW`; elements.emoji.innerHTML = style.emoji; elements.lastUpdated.innerText = `Last Updated: ${formatTimestamp(latestData.lastUpdated)}`; elements.bar.style.width = `${style.percentage}%`; elements.bar.style.backgroundColor = style.color; return; }
                let data = apiDataCache[endpoint]; if (!data) { const response = await fetch(endpoint); if (!response.ok) throw new Error(`Network error`); data = await response.json(); apiDataCache[endpoint] = data; }
                if (!Array.isArray(data) || data.length < 2) throw new Error("Invalid NOAA format");
                const headers = data[0]; const columnName = type === 'bz' ? 'bz_gsm' : type; const valueIndex = headers.indexOf(columnName); const timeIndex = headers.indexOf('time_tag'); if (valueIndex === -1 || timeIndex === -1) throw new Error(`Column not found`);
                let latestRow = null; for (let i = data.length - 1; i > 0; i--) { const row = data[i]; const val = parseFloat(row[valueIndex]); if (!isNaN(val) && val > -9999) { latestRow = row; break; } }
                ...
                const unit = type==='speed'?' km/s':type==='density'?' p/cm³': ' nT'; const style = getGaugeStyle(value, type);
                elements.value.innerText = `${value.toFixed(1)}${unit}`; elements.emoji.innerHTML = style.emoji; elements.lastUpdated.innerText = `Last Updated: ${formatTimestamp(lastUpdated)}`; elements.bar.style.width = `${style.percentage}%`; elements.bar.style.backgroundColor = style.color;
            } catch (error) { elements.value.innerText = "Error"; elements.emoji.innerHTML = GAUGE_EMOJIS.error; elements.lastUpdated.innerText = `Failed: ${(error as Error).message.substring(0,30)}`; elements.bar.style.width='0%'; }
        }
        async function renderMagneticChart() {
            const ctx = magneticChartRef.current?.getContext('2d'); if (!ctx) return;
            ...
            try {
                ...
                const filteredData = data.slice(1).map((row: any[]) => ({ time: new Date(row[timeIndex]), bt: parseFloat(row[btIndex]) > -9999 ? parseFloat(row[btIndex]) : null, bz: parseFloat(row[bzIndex]) > -9999 ? parseFloat(row[bzIndex]) : null, })).filter((p: any) => p.time >= chartTimeLimit);
                if (filteredData.length === 0) return;
                const labels = filteredData.map((p: any) => p.time.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}));
                const btValues = filteredData.map((p: any) => p.bt); const bzValues = filteredData.map((p: any) => p.bz);
        ...
        async function fetchSensor() {
            try {
                ...
                const tnrData = await tnrResponse.json(); const basicData = await basicResponse.json(); const scoreValue = parseFloat(tnrData.values.pop().value); (document.getElementById('aurora-score') as HTMLElement).innerHTML = `${scoreValue.toFixed(1)}% <span class="emoji">${getAuroraEmoji(scoreValue)}</span>`;
                const lastUpdated = new Date(basicData.values.pop().lastUpdated); (document.getElementById('last-updated') as HTMLElement).innerText = `Last Updated: ${lastUpdated.toLocaleDateString()} ${lastUpdated.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
                updateThermometer(scoreValue); updateAuroraBlurb(scoreValue);
            } catch (error) { (document.getElementById('aurora-score') as HTMLElement).innerHTML = `Error <span class="emoji">❓</span>`; (document.getElementById('last-updated') as HTMLElement).innerText = `Update Failed: ${(error as Error).message}`; updateThermometer(0); updateAuroraBlurb(-1); }
        }
        function getAuroraEmoji(s: number){return s<10?'\u{1F61F}':s<25?'\u{1F610}':s<50?'\u{1F642}':s<80?'\u{1F600}':'\u{1F929}'}
        function updateThermometer(s: number){const t=document.getElementById('thermometer-line') as HTMLElement;t.style.width=Math.min(s,100)+'%';t.style.backgroundColor=s<10?'#808080':s<25?'#FFD700':s<40?'#FFA500':s<50?'#FF4500':s<80?'#800080':'#FF1493'}
    ...
    // This is the JSX that renders the forecast dashboard's HTML structure
    return (
        <div className="container p-5 text-neutral-300">
