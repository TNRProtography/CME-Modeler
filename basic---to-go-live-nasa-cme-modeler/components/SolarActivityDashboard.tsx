...
// --- NEW: tolerant helpers for robust parsing (no caching) ---
const norm = (s: any) => (s == null ? '' : String(s)).replace(/\s+/g, '').toLowerCase();
const toTime = (s: any) => {
...
const SolarActivityDashboard: React.FC<SolarActivityDashboardProps> = ({ apiKey, setViewerMedia, setLatestXrayFlux, onViewCMEInVisualization, navigationTarget }) => {
  ...
  // --- UPDATED: Robust X-ray loader (skip bad rows, accept minor variations) ---
  const fetchXrayFlux = useCallback(() => {
    setLoadingXray('Loading X-ray flux data...');
    fetch(`${NOAA_XRAY_FLUX_URL}?_=${Date.now()}`).then(res => res.ok ? res.json() : Promise.reject(new Error(`HTTP ${res.status}`)))
      .then((raw: any[]) => {
        const grouped = new Map<number, { time: number; short: number | null }>();
        let dropped = 0;

        for (const d of raw ?? []) {
          try {
            if (!d) { dropped++; continue; }
            const e = norm(d.energy); // e.g. "0.1-0.8nm" or "0.1-0.8 nm"
            if (!e.includes('0.1-0.8nm')) continue; // ignore other channels
            const t = toTime(d.time_tag);
            const f = toNum(d.flux);
            if (t == null || f == null) { dropped++; continue; }

            if (!grouped.has(t)) grouped.set(t, { time: t, short: null });
            grouped.get(t)!.short = f;
          } catch {
            dropped++;
          }
}

        const processed = Array.from(grouped.values())
          .filter(r => r.short != null)
        ...
        if (processed.length === 0) {
          ...
          setLoadingXray('No valid X-ray data found.');
          setLastXrayUpdate(new Date().toLocaleTimeString('en-NZ'));
          console.warn('[X-ray] All rows invalid or wrong channel. Dropped:', dropped);
          return;
        }

        if (dropped > 0) console.warn(`[X-ray] Loaded with partial data: kept ${processed.length}, dropped ${dropped}.`);

        setAllXrayData(processed);
        setLoadingXray(null);
        const latestFluxValue = processed[processed.length - 1].short!;
        ...
        if (prevFlux != null) {
          ...
          const XRAY_THRESHOLDS = [
            ...
            { value: 1e-4, tag: 'flare-X1', title: 'Significant Flare: ≥X1-Class', body: (cls: string) => `A powerful ${cls} flare is in progress. Strong radio blackouts expected.` },
          ...
          XRAY_THRESHOLDS.forEach(t => {
            if (latestFluxValue >= t.value && prevFlux < t.value && canSendNotification(t.tag, 30 * 60 * 1000)) {
              sendNotification(t.title, t.body(currentClass), { tag: t.tag, data: { url: '/#goes-xray' } });
  ...
  // --- UPDATED: Robust Proton loader (skip bad rows, accept >=10 MeV variations) ---
  const fetchProtonFlux = useCallback(() => {
    setLoadingProton('Loading proton flux data...');
    fetch(`${NOAA_PROTON_FLUX_URL}?_=${Date.now()}`).then(res => res.ok ? res.json() : Promise.reject(new Error(`HTTP ${res.status}`)))
      .then((raw: any[]) => {
        let kept = 0, dropped = 0;
        const processed: { time: number; flux: number }[] = [];

        for (const d of raw ?? []) {
          try {
            if (!d) { dropped++; continue; }
            const energyStr = norm(d.energy ?? d.energy_channel);
            // Accept things like ">=10MeV", ">=10 MeV", case-insensitive
            if (!(energyStr.includes('>=10') && energyStr.includes('mev'))) continue; // ignore other channels
            const t = toTime(d.time_tag);
            const f = toNum(d.flux);
            if (t == null || f == null) { dropped++; continue; }
            processed.push({ time: t, flux: f });
            kept++;
          } catch {
            dropped++;
          }
        ...
        if (processed.length === 0) {
          ...
          console.warn('[Protons] All rows invalid or wrong channel. Dropped:', dropped);
          return;
        }

        if (dropped > 0) console.warn(`[Protons] Loaded with partial data: kept ${kept}, dropped ${dropped}.`);

        setAllProtonData(processed);
        setLoadingProton(null);
        const latestFluxValue = processed[processed.length - 1].flux;
        ...
        if (prevFlux != null) {
          if (latestFluxValue >= S1_THRESHOLD && prevFlux < S1_THRESHOLD && canSendNotification('proton-S1', 30 * 60 * 1000)) {
            sendNotification('Proton Event Alert: S1 Class!', `Proton flux (>=10 MeV) has reached S1 class (>=${S1_THRESHOLD} pfu)! Current flux: ${latestFluxValue.toFixed(2)} pfu.`, { tag: 'proton-S1', data: { url: '/#proton' } });
          ...
          if (latestFluxValue >= S3_THRESHOLD && prevFlux < S3_THRESHOLD && canSendNotification('proton-S3', 60 * 60 * 1000)) {
            sendNotification('Major Proton Event Alert: S3+ Class!', `Proton flux (>=10 MeV) has reached S3 class (>=${S3_THRESHOLD} pfu)! Current flux: ${latestFluxValue.toFixed(2)} pfu.`, { tag: 'proton-S3', data: { url: '/#proton' } });
  ...
  // --- UPDATED: Robust Flares loader (skip malformed rows) ---
  const fetchFlares = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${NASA_DONKI_BASE_URL}FLR?startDate=${startDate}&endDate=${endDate}&api_key=${apiKey}&_=${Date.now()}`);
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();

      let kept = 0, dropped = 0;
      const processedData = (Array.isArray(data) ? data : []).map((flare: any) => {
        try {
          const peakT = toTime(flare?.peakTime);
          const cls = flare?.classType ? String(flare.classType) : null;
          if (peakT == null || !cls) { dropped++; return null; }
          const hasCME = Array.isArray(flare?.linkedEvents) && flare.linkedEvents.some((e: any) => typeof e?.activityID === 'string' && e.activityID.includes('CME'));
          kept++;
          return { ...flare, hasCME };
        } catch { dropped++; return null; }
      }).filter(Boolean as any)
        .sort((a: any, b: any) => new Date(b.peakTime).getTime() - new Date(a.peakTime).getTime());
      ...
      if (!processedData.length) { 
        ...
        console.warn('[Flares] No valid flare rows. Dropped:', dropped);
        return; 
      }

      if (dropped > 0) console.warn(`[Flares] Loaded with partial data: kept ${kept}, dropped ${dropped}.`);

      setSolarFlares(processedData);
      setLoadingFlares(null);

      if (processedData.length > 0) {
        const latestFlare = processedData[0];
