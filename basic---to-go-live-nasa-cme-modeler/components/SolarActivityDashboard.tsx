...
// SDO URLs now point to the NEW Cloudflare Worker proxy and include new variants
const SDO_PROXY_BASE_URL = 'https://sdo-imagery-proxy.thenamesrock.workers.dev';
const SDO_HMI_512_URL = `${SDO_PROXY_BASE_URL}/sdo-hmi-512`;
const SDO_AIA_193_512_URL = `${SDO_PROXY_BASE_URL}/sdo-aia193-512`;
const SDO_HMI_BC_1024_URL = `${SDO_PROXY_BASE_URL}/sdo-hmibc-1024`; // New HMI Continuum
const SDO_HMI_IF_1024_URL = `${SDO_PROXY_BASE_URL}/sdo-hmiif-1024`; // New HMI Intensitygram
const SDO_AIA_193_2048_URL = `${SDO_PROXY_BASE_URL}/sdo-aia193-2048`; // New AIA 193 (Coronal Holes)

const NASA_IPS_URL = 'https://spottheaurora.thenamesrock.workers.dev/ips'; // Ensure this worker is also robustly configured!
...
const SolarActivityDashboard: React.FC<SolarActivityDashboardProps> = ({ apiKey, setViewerMedia, setLatestXrayFlux }) => {
    ...
    // Updated states for SDO images to support new resolutions
    ...
    // Added new active image options
    ...
    const tooltipContent = useMemo(() => ({
        ...
        // Updated SDO HMI/AIA tooltips to differentiate sizes/types
    ...
    useEffect(() => {
        const runAllUpdates = () => {
            ...
            fetchImage(SDO_HMI_512_URL, setSdoHmi512); // Existing 512px HMI
            fetchImage(SDO_AIA_193_512_URL, setSdoAia193_512); // Existing 512px AIA 193
            fetchImage(SDO_HMI_BC_1024_URL, setSdoHmiBc1024); // New HMI Continuum
            fetchImage(SDO_HMI_IF_1024_URL, setSdoHmiIf1024); // New HMI Intensitygram
            fetchImage(SDO_AIA_193_2048_URL, setSdoAia193_2048); // New AIA 193 (Coronal Holes)
    ...
    }, [fetchImage, fetchXrayFlux, fetchProtonFlux, fetchFlares, fetchInterplanetaryShockData]); // Added new fetch dependencies
                    ...
                    <main className="grid grid-cols-12 gap-5">
                        ...
                        <div className="col-span-12 card bg-neutral-950/80 p-4 h-[550px] flex flex-col">
                            ...
                            <div className="flex justify-center gap-2 my-2 flex-wrap mb-4">
                                ...
                                {/* Original SDO 512px options */}
                                <button
                                    onClick={() => setActiveSunImage('SDO_HMI_512')}
                                    className={`px-3 py-1 text-xs rounded transition-colors ${activeSunImage === 'SDO_HMI_512' ? 'bg-sky-600 text-white' : 'bg-neutral-700 hover:bg-neutral-600'}`}
                                    title={tooltipContent['sdo-hmi-512']}
                                >
                                    SDO HMI (512Å)
                                </button>
                                <button
                                    onClick={() => setActiveSunImage('SDO_AIA193_512')}
                                    className={`px-3 py-1 text-xs rounded transition-colors ${activeSunImage === 'SDO_AIA193_512' ? 'bg-sky-600 text-white' : 'bg-neutral-700 hover:bg-neutral-600'}`}
                                    title={tooltipContent['sdo-aia193-512']}
                                >
                                    SDO AIA 193Å (512px)
                                </button>
                                {/* NEW SDO 1024px & 2048px options */}
                                <button
                            ...
                            <div className="flex-grow flex justify-center items-center relative min-h-0">
                                ...
                                {/* Existing SDO 512px renders */}
                                {activeSunImage === 'SDO_HMI_512' && (
                                    <div
                                        onClick={() => sdoHmi512.url !== '/placeholder.png' && sdoHmi512.url !== '/error.png' && setViewerMedia({ url: sdoHmi512.url, type: 'image' })}
                                        className="flex-grow flex justify-center items-center cursor-pointer relative min-h-0 w-full h-full"
                                        title={tooltipContent['sdo-hmi-512']}
                                    >
                                        <img src={sdoHmi512.url} alt="SDO HMI 512px" className="max-w-full max-h-full object-contain rounded-lg"/>
                                        {sdoHmi512.loading && <p className="absolute text-neutral-400 italic">{sdoHmi512.loading}</p>}
                                    </div>
                                )}
                                {activeSunImage === 'SDO_AIA193_512' && (
                                    <div
                                        onClick={() => sdoAia193_512.url !== '/placeholder.png' && sdoAia193_512.url !== '/error.png' && setViewerMedia({ url: sdoAia193_512.url, type: 'image' })}
                                        className="flex-grow flex justify-center items-center cursor-pointer relative min-h-0 w-full h-full"
                                        title={tooltipContent['sdo-aia193-512']}
                                    >
                                        <img src={sdoAia193_512.url} alt="SDO AIA 193Å 512px" className="max-w-full max-h-full object-contain rounded-lg"/>
                                        {sdoAia193_512.loading && <p className="absolute text-neutral-400 italic">{sdoAia193_512.loading}</p>}
                                    </div>
                                )}
